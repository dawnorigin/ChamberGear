...
{
  //extern SemaphoreHandle_t xBottonSemaphore;
  ...
  if (EXTI_GetITStatus(EXTI_Line0) != RESET) {
    ...
    //xSemaphoreGiveFromISR(xBottonSemaphore, &xHigherPriorityTaskWoken);
...
{
  extern QueueHandle_t xSwitchQueue;
  portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
  char number = 1;
  ...
  if (EXTI_GetITStatus(EXTI_Line1) != RESET) {
    /* Disable the interrupt */
    EXTI->IMR &= ~ EXTI_Line1;
    /* Send a switch event to queue */
    xQueueSendFromISR( xSwitchQueue, &number, &xHigherPriorityTaskWoken );
    /* Clear interrupt pending bit */
...
void EXTI9_5_IRQHandler(void)
{
  ...
  if (flag != RESET) {
    ...
    for (number = 5; number < 10; number++) {
      if ((flag & (EXTI_Line0 << number)) != RESET) {
        /* Send a switch event to queue */
        xQueueSendFromISR( xSwitchQueue, &number, &xHigherPriorityTaskWoken );
      }
    }
